...
async function loadSubmissionList() {
    const params = new URLSearchParams(window.location.search);
    const experimentSlug = params.get("experiment");

    pageTitle.textContent = "Submissions";
    metaBox.textContent = experimentSlug
        ? `Recent submissions for ${experimentSlug}.`
        : "Recent submissions visible to your account.";
    contentBox.innerHTML = "<p>Loading submissions...</p>";

    const path = experimentSlug
        ? `/submissions/?experiment=${encodeURIComponent(experimentSlug)}`
        : "/submissions/";

    const submissions = await apiFetch(path);
