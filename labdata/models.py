...
class DataField(models.Model):
    ...
    # Backward-compatible convenience relation. Source of truth is table.experiment.
    experiment = models.ForeignKey(
        Experiment,
        on_delete=models.CASCADE,
        related_name="fields",
    ...
    variable_name = models.SlugField(
        ...
        blank=True,
        help_text=(
            "Formula variable name, for example length, time_period, theta. "
            "Auto-filled from the field name if blank."
        ),
    )
    ...
    calculation_formula = models.CharField(
        max_length=500,
        blank=True,
        help_text="Only for derived fields. Examples: length/time, 2*pi*r, sin(theta*pi/180).",
    )
    ...
    def clean(self):
        ...
        if self.variable_name and "-" in self.variable_name:
            raise ValidationError(
                {"variable_name": "Use underscores instead of hyphens in formula variable names."}
            )
        ...
        if self.table_id:
            ...
            duplicate = (
                DataField.objects
                .filter(table=self.table, variable_name=variable_name)
                .exclude(pk=self.pk)
            )
            if duplicate.exists():
                raise ValidationError(
                    {"variable_name": "This variable name is already used in this table."}
                )
...
class PlotConfig(models.Model):
    ...
    show_fit_equation = models.BooleanField(default=True)
    ...
    def clean(self):
        ...
        if self.x_field_id and self.table_id and self.x_field.table_id != self.table_id:
            raise ValidationError({"x_field": "X field must belong to the selected table."})

        if self.y_field_id and self.table_id and self.y_field.table_id != self.table_id:
            raise ValidationError({"y_field": "Y field must belong to the selected table."})
