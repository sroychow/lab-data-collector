...
def experiment_create(request):
    if not request.user.is_staff:
        messages.error(
            request,
            "Only staff users can create experiments. Use Django admin for table and field setup.",
        )
    ...
    if form.is_valid():
        ...
        messages.success(
            request,
            "Experiment created. Add tables, fields and final results from the admin panel.",
        )
...
def experiment_detail(request, slug=None, experiment_id=None):
    ...
    return render(
        request,
        "labdata/experiment_detail.html",
        {
...
def submit_data(request, slug):
    ...
    form = DynamicTableSubmissionForm(
...
@login_required
def submission_detail(request, pk=None, submission_id=None):
    submission_pk = pk if pk is not None else submission_id
    ...
    plot_configs = (
        PlotConfig.objects
        .filter(table__experiment=submission.experiment, is_active=True)
    ...
    return render(
        request,
        "labdata/submission_detail.html",
        {
            "submission": submission,
...
@login_required
def submission_plot_detail(request, pk=None, submission_id=None, plot_id=None):
    submission_pk = pk if pk is not None else submission_id
...
def plot_detail(request, plot_id):
    ...
    submission = (
        Submission.objects
        .filter(experiment=plot_config.table.experiment)
        .order_by("-created_at")
        .first()
    )
...
def export_csv(request, slug):
    ...
    header.extend(
        [
            f"Result::{rf.name} ({rf.unit})" if rf.unit else f"Result::{rf.name}"
        ...
        for row in sub.rows.all():
            ...
            writer.writerow(
                [
                    ...
                    row.serial_number,
                    *[
                        value_map.get(field_id, "") if table_id == row.table_id else ""
                        for table_id, field_id, _ in all_field_headers
                    ],
